      ...
      # Доступные команды 
        ...
        for key in users:
            if (users[key])['id'] == id:
                ...
                admin_nick = (users[key])['nick']
                break
        help = ["/time","/random","/cp","/changepassword","/changepass","/info","/online","/music"]    
        if admin:
            for item in ["/say","/erase","/setadmin","/sa","/removeadmin","/ra","/clear"]:
                help.append(item)
            
        self.send(json.dumps(['commands',help]).encode())
